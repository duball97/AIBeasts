  ...
  const formatResponse = (responseText) => {
    try {
      ...
      const topText =
        lines.find((line) => line.toLowerCase().startsWith("top:"))?.replace(/top:/i, "").trim() || "No top text";
      const bottomText =
        lines.find((line) => line.toLowerCase().startsWith("bottom:"))?.replace(/bottom:/i, "").trim() || "No bottom text";
